...
Like the other correlation wrappers, `pcorr()` returns an estimate matrix by
default. Use `estimate()`, `tidy()`, `ci()`, and `confint()` for public access.
If you need the covariance or precision matrix, set `return_details = TRUE`.
